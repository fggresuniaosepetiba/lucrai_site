...
- [x] Senhas: todos criados com `123` (`PasswordHasher` + `MustChangePassword=true`); `AuthController:48` exige troca via `POST /api/auth/change-password` (flag vem em `LoginResponse.user.mustChangePassword`); `DataSeeder:118-122` só reativa `MustChangePassword=true` quando hash ainda é `123` — após troca (`ChangePassword`) fica `false` e não reseta no reboot (verificado 2026-09-07 20:46 UTC pós zero via Npgsql Opção A: `lucrai.adm 7671c516…` + `joao.ribeiro bb112dcd…` zerados para `123` `MustChangePassword=true` `SecurityStamp`/`ConcurrencyStamp` novos + `RefreshTokens` deletados `13/2`; `POST /api/auth/login` com `123` → `200 {mustChangePassword:true}` vai para `POST /api/auth/change-password 200`; antigas `@Lucrai2026` agora `401`)
